...
FILE_A = r'C:\Users\vince\OneDrive\Documents\Conflixis\conflixis-ai\company-data\op_compare\input_file_a\ByfH69KTozhg7BV05BWI_disclosures_entitynames_v2.csv'
# This is the OP file
FILE_B = r'C:\Users\vince\OneDrive\Documents\Conflixis\conflixis-ai\company-data\op_compare\input_op\bquxjob_7b6211b8_1934f852c71_2023_gpoidnames.csv'

COLUMN_NAME_A = 'external_entity_name'  # Default column name for entity names in FILE_A
...
def match_name_to_df(name: str, name_A: str, row_A: pd.Series, df_B: pd.DataFrame, column_name_b: str) -> dict:
    ...
    result_dict = row_A.to_dict()  # Ensure all columns from row_A are included, including `document_id`
    result_dict.update({
        "Name A PP": name,
        "ID B": None,
        "Name B": None,
...
if __name__ == "__main__":
    ...
    if not matched_df.empty:
        ...
        reordered_columns = ['document_id', COLUMN_NAME_A, 'GPOName_B', 'Name A PP', 'Name B PP'] + \
                            [col for col in matched_df.columns if
                             col not in ['document_id', COLUMN_NAME_A, 'GPOName_B', 'Name A PP', 'Name B PP']]
        ...
        output_file = f"C:\\Users\\vince\\OneDrive\\Documents\\Conflixis\\conflixis-ai\\company-data\\op_compare\\output_file_b\\{file_a_base_name}_matched_{unique_id}.csv"
