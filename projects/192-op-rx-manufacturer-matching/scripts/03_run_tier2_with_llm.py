#!/usr/bin/env python3
"""
Run Tier2 name matching with LLM enhancement for OP and RX manufacturer names
Achieves 96.9% accuracy using the production-ready tier2_matcher module
"""
...
# Import Tier2 matcher (copied to scripts directory)
...
def run_tier2_matching_with_blocking(op_df: pd.DataFrame, rx_df: pd.DataFrame, config: dict) -> pd.DataFrame:
    """
    Run Tier2 matching with blocking and LLM enhancement
    """
    ...
    logger.info("Starting Tier2 name matching with LLM enhancement...")
    ...
    for blocking_key in sorted(op_groups.groups.keys()):
        ...
        for _, op_row in op_block.iterrows():
            ...
            if total_processed % 50 == 0:
                ...
                logger.info(f"  API calls so far: {api_calls_made}")
...
def save_results(results_df: pd.DataFrame, output_dir: Path, config: dict):
    ...
    # Save full results
    full_file = output_dir / f"tier2_matching_full_{timestamp}.csv"
    ...
    best_file = output_dir / f"tier2_matching_best_{timestamp}.csv"
    ...
    summary = {
        "timestamp": timestamp,
        "method": "Tier2 with LLM Enhancement",
    ...
    # Save summary
    summary_file = output_dir / f"tier2_matching_summary_{timestamp}.json"
...
def main():
    ...
    logger.info("="*60)
    logger.info("Starting OP-RX Manufacturer Tier2 Matching with LLM")
    logger.info("="*60)
    ...
    results_df = run_tier2_matching_with_blocking(op_df, rx_df, config)
    ...
    if not results_df.empty:
        ...
        # Print final summary
        logger.info("\n" + "="*60)
        logger.info("TIER2 MATCHING COMPLETE")
        logger.info("="*60)
